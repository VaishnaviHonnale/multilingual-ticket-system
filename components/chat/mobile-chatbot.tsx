...
export function MobileChatbot({ className }: MobileChatbotProps) {
  ...
  if (!isOpen) {
    return (
      <div className={cn("fixed bottom-4 right-4 z-50 md:hidden", className)}>
        ...
        <Badge 
          variant="destructive" 
          className="absolute -top-1 -right-1 h-5 w-5 rounded-full p-0 flex items-center justify-center animate-pulse text-xs"
        >
          AI
        </Badge>
      </div>
